...
# remove outliner
low_perct  = quantile(ff_input$MTY_RTN_TRANSITTIME_ACTUAL, .25) - 0.5*IQR(ff_input$MTY_RTN_TRANSITTIME_ACTUAL)
high_perct = quantile(ff_input$MTY_RTN_TRANSITTIME_ACTUAL, .75) + 0.5*IQR(ff_input$MTY_RTN_TRANSITTIME_ACTUAL)

cat("BEfore remove outlier,", dim(ff_input)[1], '\n')
#ff_input = ff_input[ff_input$MTY_RTN_TRANSITTIME_ACTUAL > low_perct & ff_input$MTY_RTN_TRANSITTIME_ACTUAL< high_perct,]
...
describe(ff_input)
str(ff_input)
#cat('Rows before remove duplicated record=', dim(ff_input)[1], '\n')
# ff_input =ff_input[!duplicated(subset(ff_input, select = c('ORIG_IODT','DEST_IODT','LOCATION','STATUS', 'EVENTDATE_UTC_IODT', 'EVENTDATE_UTC', 'EVENTDATE'))),]
#cat('Rows after remove duplicated record=', dim(ff_input)[1], '\n')

# Generate features
# convert to hours
ff_input$TRAVEL_TIME_HOURS = ff_input$MTY_RTN_TRANSITTIME_ACTUAL
# TO MEASURE CURRENT ETA
#ff_input$TRAVEL_TIME_HOURS = ff_input$MTY_RTN_TRANSITTIME_ACTUAL_EST/60.0
